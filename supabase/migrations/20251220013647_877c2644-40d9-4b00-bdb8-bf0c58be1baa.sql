-- Fix auto_apply_late_penalty to charge hourly overage for FULL late time (not just beyond 3 hours)
...
BEGIN
  ...
  IF NEW.status = 'completed' 
    ...
    -- Calculate lateness using end_time_utc
    ...
    -- Only apply penalty if 3+ hours late (auto-close threshold)
    IF minutes_late >= 180 THEN
      
      -- Calculate hourly overage for FULL late time (not just beyond 3 hours)
      hours_late := CEILING(minutes_late / 60.0);
      hourly_overage := hours_late * COALESCE(spot_price, 6.00);
      
      -- Fee calculations - platform gets 7% of hourly overage
      ...
      penalty_description := format('Auto-close: $%s penalty + $%s overage (%shr @ $%s/hr)', base_penalty, hourly_overage, hours_late, COALESCE(spot_price, 6.00));
      ...
      RAISE NOTICE 'Penalty applied: booking %, amount $%, hours late %', NEW.id, total_charge_amount, hours_late;
