...
BEGIN
  ...
  FOR hour_val IN 6..23 LOOP
    FOR minute_val IN 0..1 LOOP
      -- Create slot timestamp in spot's timezone (30-min intervals: 0 or 30 minutes)
      slot_start := (p_date || ' ' || LPAD(hour_val::TEXT, 2, '0') || ':' || LPAD((minute_val * 30)::TEXT, 2, '0') || ':00')::timestamp;
      ...
      result := result || jsonb_build_object(
        'time', LPAD(hour_val::TEXT, 2, '0') || ':' || LPAD((minute_val * 30)::TEXT, 2, '0'),
