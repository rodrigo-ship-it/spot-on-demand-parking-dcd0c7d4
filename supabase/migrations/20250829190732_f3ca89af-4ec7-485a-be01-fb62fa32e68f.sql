  ...
  FOR hour_val IN 6..21 LOOP
    FOR minute_val IN 0..1 LOOP
      ...
      IF (hour_val + p_duration_hours) <= 23 THEN
        ...
        -- Check if any existing booking conflicts with this slot
        ...
        WHERE spot_id = p_spot_id
          ...
          AND (
            ...
            (slot_start_timestamp - INTERVAL '30 minutes') < (end_time_utc + INTERVAL '30 minutes')
            AND
            -- New slot buffer end > existing booking buffer start  
            (slot_end_timestamp + INTERVAL '30 minutes') > (start_time_utc - INTERVAL '30 minutes')
          );
