  ...
  FOR hour_val IN 6..21 LOOP
    FOR minute_val IN 0..1 LOOP
      ...
      IF (hour_val + p_duration_hours) <= 23 THEN
        ...
        -- Check if any existing booking conflicts with this slot (EXACTLY 30 minutes before and after)
        ...
        WHERE spot_id = p_spot_id
          ...
          -- Booking conflicts if it overlaps with the slot (30-minute buffer on each side)
          AND (
            -- Slot overlaps with booking + 30min buffer
            (slot_start_timestamp - INTERVAL '30 minutes') < end_time_utc
            AND (slot_end_timestamp + INTERVAL '30 minutes') > start_time_utc
          );
