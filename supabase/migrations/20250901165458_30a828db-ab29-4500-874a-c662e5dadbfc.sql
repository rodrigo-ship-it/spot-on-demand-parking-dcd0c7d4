-- Fix Critical Security Vulnerabilities in RLS Policies (Corrected)
-- This migration addresses the security issues found by the security scanner

-- 1. CRITICAL: Fix profiles table security issues
...
-- 4. WARNING: Fix bookings table - ensure proper access control
...
DROP POLICY IF EXISTS "Users can view disputes they created" ON public.disputes;
DROP POLICY IF EXISTS "Spot owners can view disputes for their spots" ON public.disputes;
...
DROP POLICY IF EXISTS "Owners can view their own spots with full details" ON public.parking_spots;
...
-- 8. Create function to get safe public profile data (if it doesn't exist)
