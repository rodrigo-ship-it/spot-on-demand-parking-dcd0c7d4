...
export const MapComponent = ({ spots, onSpotSelect, centerLocation }: MapComponentProps) => {
          ...
          map.current.on('load', () => {
            ...
            groupedSpots.forEach((group) => {
              ...
              if (isPremiumMarker) {
                ...
                // Create custom HTML marker for non-premium spots (no shadow)
                const el = document.createElement('div');
                el.className = 'custom-marker';
                el.style.cssText = `
                  width: 28px;
  ...
  useEffect(() => {
    ...
    groupedSpots.forEach((group) => {
      ...
      // Create custom HTML marker for all spots (no shadow circles)
      const el = document.createElement('div');
      el.className = 'custom-marker';
      el.style.cssText = `
        width: ${isPremiumMarker ? '34px' : '28px'};
        height: ${isPremiumMarker ? '34px' : '28px'};
        border-radius: 50%;
        ${isPremiumMarker ? 'border: 3px solid #d4af37;' : ''}
        background-color: ${getPinColor(primarySpot, isMultipleSpots)};
        cursor: pointer;
        display: flex;
        align-items: center;
