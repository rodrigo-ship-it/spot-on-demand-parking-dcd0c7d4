-- Fix the auto_apply_late_penalty trigger to prevent duplicate penalties
-- The issue is the trigger fires on every UPDATE, causing multiple penalty charges

-- First, drop the existing trigger if it exists
DROP TRIGGER IF EXISTS auto_apply_late_penalty_trigger ON bookings;

-- Recreate the trigger function with better duplicate prevention
...
DECLARE
  ...
  max_charge_amount NUMERIC := 70.00; -- $70 cap
...
BEGIN
  -- CRITICAL: Only proceed if this was a system completion and status changed from non-completed to 'completed'
  -- AND prevent duplicate penalty application
  IF NEW.status = 'completed' 
    ...
    WHERE booking_id = NEW.id AND credit_type = 'late_checkout';
    
    -- If penalty already exists, skip to prevent duplicates
    IF existing_penalty_count > 0 THEN
      RAISE NOTICE 'Penalty already exists for booking %, skipping duplicate penalty application', NEW.id;
      RETURN NEW;
    END IF;
    ...
    IF minutes_late >= 180 THEN
      ...
      -- Calculate base penalty (stays with platform) - CORRECTED VALUES
      ...
      penalty_description := format('Auto-close: $%s penalty + $%s overage + fees/taxes (Total: $%s, %s offense)', 
        base_penalty, hourly_overage, total_charge_amount, 
        CASE WHEN is_first_offense THEN 'first' ELSE 'repeat' END);
      ...
      INSERT INTO penalty_credits (
        ...
        NEW.renter_id,
        NEW.id,
        total_charge_amount, -- Use the total charge amount here (capped at $70)
        'late_checkout',
        penalty_description,
        'active'
      ...
      RAISE NOTICE 'Penalty applied for booking % - Amount: $%, Description: %', NEW.id, total_charge_amount, penalty_description;
      ...
      SELECT net.http_post(
        ...
        body := jsonb_build_object(
          'bookingId', NEW.id,
          'amount', total_charge_amount, -- Pass the TOTAL amount including all fees (capped at $70)
...
-- Create the trigger to fire ONLY on booking updates
CREATE TRIGGER auto_apply_late_penalty_trigger
    AFTER UPDATE ON bookings
    FOR EACH ROW
    EXECUTE FUNCTION auto_apply_late_penalty();

-- Clean up duplicate penalty credits (keep the latest one per booking)
WITH duplicate_penalties AS (
  SELECT id, 
         ROW_NUMBER() OVER (PARTITION BY booking_id ORDER BY created_at DESC) as rn
  FROM penalty_credits 
  WHERE credit_type = 'late_checkout'
    AND created_at > NOW() - INTERVAL '7 days'  -- Only clean recent duplicates
)
UPDATE penalty_credits 
SET status = 'cancelled', 
    description = description || ' (DUPLICATE - CANCELLED)'
WHERE id IN (
  SELECT id FROM duplicate_penalties WHERE rn > 1
) AND status = 'active';

-- Log this fix
SELECT public.log_security_event_enhanced(
  'penalty_system_fixed',
  jsonb_build_object(
    'action', 'fixed_duplicate_penalty_trigger',
    'timestamp', extract(epoch from now()),
    'description', 'Fixed auto_apply_late_penalty trigger to prevent duplicate charges'
  ),
  NULL,
  'critical'
);