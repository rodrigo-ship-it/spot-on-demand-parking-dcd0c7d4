-- Fix timezone comparison in get_available_time_slots function (correct UTC conversion)
...
DECLARE
  ...
  slot_start_utc TIMESTAMP WITH TIME ZONE;
  slot_end_utc TIMESTAMP WITH TIME ZONE;
BEGIN
  ...
  FOR hour_val IN 6..21 LOOP
    FOR minute_val IN 0..1 LOOP
      ...
      IF (hour_val + p_duration_hours) <= 23 THEN
        ...
        -- Convert slot timestamps to UTC for comparison
        slot_start_utc := slot_start_timestamp AT TIME ZONE 'UTC';
        slot_end_utc := slot_end_timestamp AT TIME ZONE 'UTC';
        
        -- Count occupied spots during this time slot (only confirmed/active bookings)
        -- Compare everything in UTC
        SELECT COUNT(*) INTO occupied_count
        FROM bookings
        WHERE spot_id = p_spot_id
          AND status IN ('confirmed', 'active')
          AND (
            -- Check new timezone-aware columns first (both in UTC)
            (start_time_utc IS NOT NULL AND end_time_utc IS NOT NULL AND
             slot_start_utc < end_time_utc AND start_time_utc < slot_end_utc)
            OR
