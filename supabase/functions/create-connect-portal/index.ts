...
serve(async (req) => {
  ...
  try {
    ...
    // Create account link for managing the Connect account
    const accountLink = await stripe.accountLinks.create({
      account: payoutSettings.stripe_connect_account_id,
      refresh_url: `${req.headers.get("origin")}/profile`,
      return_url: `${req.headers.get("origin")}/profile?updated=true`,
      type: "account_update",
    });
    ...
    console.error("Error creating Connect portal:", error);
