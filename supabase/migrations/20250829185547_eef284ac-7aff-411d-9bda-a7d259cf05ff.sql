CREATE OR REPLACE FUNCTION public.get_available_time_slots(p_spot_id uuid, p_date date, p_duration_hours integer DEFAULT 1, p_timezone text DEFAULT 'America/New_York'::text)
 RETURNS text
 LANGUAGE plpgsql
  ...
  FOR hour_val IN 6..21 LOOP
    FOR minute_val IN 0..1 LOOP
      ...
      IF (hour_val + p_duration_hours) <= 23 THEN
        ...
        -- Check if any existing booking conflicts with this slot (including 30-minute grace periods)
        ...
        WHERE spot_id = p_spot_id
          ...
          -- Booking conflicts if it overlaps with the slot + grace periods
          AND (start_time_utc - INTERVAL '30 minutes') < slot_end_timestamp
          AND start_time_utc < (slot_end_timestamp + INTERVAL '30 minutes')
          AND (end_time_utc + INTERVAL '30 minutes') > slot_start_timestamp
          AND end_time_utc > (slot_start_timestamp - INTERVAL '30 minutes');
