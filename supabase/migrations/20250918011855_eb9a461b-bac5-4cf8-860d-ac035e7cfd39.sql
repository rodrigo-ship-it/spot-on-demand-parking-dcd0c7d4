-- Add secure banking data functions (Fixed Trigger Syntax)

-- Trigger to automatically log access to banking data (only on write operations)
...
AS $$
BEGIN
  -- Log write operations to payout_settings table
...
-- Create trigger for banking data write auditing (INSERT, UPDATE, DELETE only)
DROP TRIGGER IF EXISTS audit_payout_settings_access ON payout_settings;
CREATE TRIGGER audit_payout_settings_write
