-- Add monthly_price column to parking_spots table
ALTER TABLE public.parking_spots 
ADD COLUMN monthly_price numeric;

-- Update pricing_type to include monthly option
...
ALTER TABLE public.parking_spots 
DROP CONSTRAINT IF EXISTS parking_spots_pricing_type_check;

-- Add new constraint with monthly option
ALTER TABLE public.parking_spots 
ADD CONSTRAINT parking_spots_pricing_type_check 
CHECK (pricing_type IN ('hourly', 'daily', 'one_time', 'monthly'));

-- Update auto_apply_late_penalty function to exclude monthly bookings
...
DECLARE
  ...
  base_penalty NUMERIC := 20.00; -- Always $20 base penalty
...
BEGIN
  ...
  IF NEW.status = 'completed' 
    ...
    -- Get spot pricing type to exclude monthly bookings from late fees
    SELECT price_per_hour, pricing_type INTO spot_price, spot_pricing_type
    FROM parking_spots 
    WHERE id = NEW.spot_id;
    
    -- SKIP LATE PENALTIES FOR MONTHLY BOOKINGS
    IF spot_pricing_type = 'monthly' THEN
      RAISE NOTICE 'Skipping late penalty for monthly booking %, monthly bookings are exempt', NEW.id;
      RETURN NEW;
    END IF;
    ...
    -- Calculate how late the checkout is using LOCAL times consistently
    ...
    IF minutes_late >= 180 THEN
      ...
      -- Base penalty is always $20 (no variation for repeat offenses)
      base_penalty := 20.00;
      ...
      -- Create description with correct $20 penalty amount
      ...
      SELECT net.http_post(
        ...
        body := jsonb_build_object(
          ...
          'penaltyAmount', base_penalty, -- Now correctly $20
