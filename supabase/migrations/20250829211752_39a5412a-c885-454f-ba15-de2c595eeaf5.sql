-- Fix the auto_apply_late_penalty trigger to use correct timezone comparisons
...
BEGIN
  ...
  IF NEW.status = 'completed' 
    ...
    -- Calculate how late the checkout is (in minutes) using UTC times for consistency
    -- Compare current UTC time with the UTC end time stored in the database
    minutes_late := EXTRACT(EPOCH FROM (NEW.updated_at - NEW.end_time_utc)) / 60;
