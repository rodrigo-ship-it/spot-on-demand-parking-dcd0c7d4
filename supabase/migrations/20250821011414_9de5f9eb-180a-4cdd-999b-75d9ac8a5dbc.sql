-- Update the auto penalty function to include proper taxes, fees, and revenue splits
  ...
  IF NEW.status = 'completed' AND NEW.completed_by_system = TRUE AND OLD.status != 'completed' THEN
    ...
    IF minutes_late >= 180 THEN
      ...
      penalty_description := format('Auto-close: $%s penalty + $%s overage + fees/taxes (Total: $%s)', 
        base_penalty, hourly_overage, base_penalty + total_overage_with_fees);
      ...
      INSERT INTO penalty_credits (
        ...
        NEW.renter_id,
        NEW.id,
        base_penalty + total_overage_with_fees,
        'late_checkout',
        penalty_description,
        'active'
      ...
      SET failed_checkouts = COALESCE(failed_checkouts, 0) + 1,
          total_penalty_credits = COALESCE(total_penalty_credits, 0) + (base_penalty + total_overage_with_fees),
          last_violation_at = NOW()
      WHERE user_id = NEW.renter_id;
      
      -- Call charge-penalty function with proper breakdown
      SELECT net.http_post(
        ...
        body := jsonb_build_object(
          'bookingId', NEW.id,
          'amount', base_penalty + total_overage_with_fees,
