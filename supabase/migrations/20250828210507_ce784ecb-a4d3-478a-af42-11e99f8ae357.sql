-- Fix timezone comparison in get_available_time_slots function
  ...
  FOR hour_val IN 6..21 LOOP
    FOR minute_val IN 0..1 LOOP
      ...
      IF (hour_val + p_duration_hours) <= 23 THEN
        ...
        -- Convert everything to UTC for proper comparison
        SELECT COUNT(*) INTO occupied_count
        FROM bookings
        WHERE spot_id = p_spot_id
          AND status IN ('confirmed', 'active')
          AND (
            -- Check new timezone-aware columns first (both converted to UTC)
            (start_time_utc IS NOT NULL AND end_time_utc IS NOT NULL AND
             slot_start_timestamp AT TIME ZONE 'UTC' < end_time_utc AND 
             start_time_utc < slot_end_timestamp AT TIME ZONE 'UTC')
            OR
