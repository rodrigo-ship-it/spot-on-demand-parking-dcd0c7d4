-- Update get_available_time_slots function to handle timezone conversion properly
  ...
  FOR hour_val IN 6..21 LOOP
    FOR minute_val IN 0..1 LOOP
      ...
      IF (hour_val + p_duration_hours) <= 23 THEN
        ...
        -- Convert stored UTC times to local timezone before extracting hours/minutes
        SELECT COUNT(*) INTO occupied_count
        FROM bookings
        WHERE spot_id = p_spot_id
          AND status IN ('confirmed', 'active')
          AND DATE(start_time AT TIME ZONE 'UTC' AT TIME ZONE CURRENT_SETTING('TIMEZONE')) = p_date
          AND (
            -- Convert UTC times to local timezone before extracting time components
            EXTRACT(HOUR FROM start_time AT TIME ZONE 'UTC' AT TIME ZONE CURRENT_SETTING('TIMEZONE')) * 60 + 
            EXTRACT(MINUTE FROM start_time AT TIME ZONE 'UTC' AT TIME ZONE CURRENT_SETTING('TIMEZONE')) < 
            (hour_val + p_duration_hours) * 60 + (minute_val * 30) + 30
            AND
            EXTRACT(HOUR FROM end_time AT TIME ZONE 'UTC' AT TIME ZONE CURRENT_SETTING('TIMEZONE')) * 60 + 
            EXTRACT(MINUTE FROM end_time AT TIME ZONE 'UTC' AT TIME ZONE CURRENT_SETTING('TIMEZONE')) + 30 > 
